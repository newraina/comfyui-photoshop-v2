        ...
        with open(ImgDir, "rb") as img_file:
            if os.path.exists(MaskDir):
                with open(MaskDir, "rb") as mask_file:
                    return base64.b64encode(mask_file.read()).decode("utf-8") + base64.b64encode(
                        img_file.read()
                    ).decode("utf-8")
            else:
...
NODE_DISPLAY_NAME_MAPPINGS = {"🔹 Photoshop RemoteConnection": "🔹Photoshop RemoteConnection(no plugin)"}
