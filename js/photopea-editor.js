...
app.registerExtension({
  ...
  async beforeRegisterNodeDef(nodeType, nodeInfo, appInstance) {
    ...
    if (
      Array.isArray(nodeInfo.output) &&
      (nodeInfo.output.includes("MASK") || nodeInfo.output.includes("IMAGE")) &&
      nodeInfo.name !== "🔹Photoshop ComfyUI Plugin"
    ) {
      appendMenuOption(nodeType, function (_, menuOptions) {
        menuOptions.unshift({
...
function addNode(nodeType, referenceNode, options) {
  ...
  newNode.pos = [
    options.before ? referenceNode.pos[0] - newNode.size[0] - 30 : referenceNode.pos[0] + referenceNode.size[0] + 30,
    referenceNode.pos[1] + options.shiftY,
  ];
